...
export async function spawnWslSidecar(
  ...
  const script = [
    ...
    "export PATH",
    "export WSLENV=",
    "export OPEN_COAI_EXPERIMENTAL_DISABLE_FILEWATCHER=true",
    "export OPEN_COAI_CLIENT=desktop",
    `export OPEN_COAI_SERVER_USERNAME=${shellEscape(username)}`,
    `export OPEN_COAI_SERVER_PASSWORD=${shellEscape(password)}`,
