...
export const McpAuthCommand = effectCmd({
  ...
  handler: Effect.fn("Cli.mcp.auth")(function* (args) {
    ...
    if (servers.length === 0) {
      ...
      prompts.log.info("Remote MCP servers support OAuth by default. Add a remote server in open-coai.json:")
      prompts.log.info(`
  "mcp": {
    "my-server": {
      "type": "remote",
...
async function resolveConfigPath(baseDir: string, global = false) {
  ...
  const candidates = [path.join(baseDir, "open-coai.json"), path.join(baseDir, "open-coai.jsonc")]

  if (!global) {
    candidates.push(path.join(baseDir, ".open-coai", "open-coai.json"), path.join(baseDir, ".open-coai", "open-coai.jsonc"))
  }
  ...
  // Default to open-coai.json if none exist
  return candidates[0]
}
