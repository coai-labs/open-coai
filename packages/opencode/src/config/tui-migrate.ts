...
/**
 * Migrates tui-specific keys (theme, keybinds, tui) from open-coai.json files
...
async function opencodeFiles(input: { directories: string[]; cwd: string }) {
  const files = [
    ...
    ...(await Filesystem.findUp(["open-coai.json", "open-coai.jsonc"], input.cwd, undefined, { rootFirst: true })),
  ]
  for (const dir of unique(input.directories)) {
    files.push(...ConfigPaths.fileInDirectory(dir, "opencode"))
  }
  if (Flag.OPEN_COAI_CONFIG) files.push(Flag.OPEN_COAI_CONFIG)
