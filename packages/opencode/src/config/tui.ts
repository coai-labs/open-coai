...
const loadState = Effect.fn("TuiConfig.loadState")(function* (ctx: { directory: string }) {
  ...
  // Every config dir we may read from: global config dir, any `.open-coai`
  // folders between cwd and home, and OPEN_COAI_CONFIG_DIR.
  const directories = yield* ConfigPaths.directories(ctx.directory)
  yield* Effect.promise(() => migrateTuiConfig({ directories, cwd: ctx.directory }))

  const projectFiles = Flag.OPEN_COAI_DISABLE_PROJECT_CONFIG ? [] : yield* ConfigPaths.files("tui", ctx.directory)
  ...
  // 2. Explicit OPEN_COAI_TUI_CONFIG override, if set.
  if (Flag.OPEN_COAI_TUI_CONFIG) {
    const configFile = Flag.OPEN_COAI_TUI_CONFIG
  ...
  // 4. `.open-coai` directories (and OPEN_COAI_CONFIG_DIR) discovered while
  // walking up the tree. Also returned below so callers can install plugin
  // dependencies from each location.
  const dirs = unique(directories).filter((dir) => dir.endsWith(".open-coai") || dir === Flag.OPEN_COAI_CONFIG_DIR)

  for (const dir of dirs) {
    if (!dir.endsWith(".open-coai") && dir !== Flag.OPEN_COAI_CONFIG_DIR) continue
