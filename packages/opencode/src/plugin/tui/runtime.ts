...
function createThemeInstaller(
  ...
  return async (file) => {
    ...
    const local_dir =
      path.basename(source_dir) === ".open-coai"
        ? path.join(source_dir, "themes")
        : path.join(source_dir, ".open-coai", "themes")
...
function defaultPluginOrigin(state: RuntimeState, spec: string): ConfigPlugin.Origin {
  return {
    spec,
    scope: "local",
    source: state.api.state.path.config || path.join(state.directory, ".open-coai", "tui.json"),
  }
}
...
async function load(input: {
  ...
  try {
    ...
    const records = Flag.OPEN_COAI_PURE ? [] : pluginOrigins
    if (Flag.OPEN_COAI_PURE && pluginOrigins.length) {
    }
