...
`,
  ...
  const pure = process.env.OPEN_COAI_PURE
  const meta = process.env.OPEN_COAI_PLUGIN_META_FILE
  process.env.OPEN_COAI_PURE = "1"
  process.env.OPEN_COAI_PLUGIN_META_FILE = tmp.extra.meta
  ...
  try {
    ...
    wait.mockRestore()
    if (pure === undefined) {
      delete process.env.OPEN_COAI_PURE
    } else {
      process.env.OPEN_COAI_PURE = pure
    }
    if (meta === undefined) {
      delete process.env.OPEN_COAI_PLUGIN_META_FILE
    } else {
      process.env.OPEN_COAI_PLUGIN_META_FILE = meta
    }
  }
})
