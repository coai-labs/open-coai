...
it.instance("plan agent denies edits except .opencode/plans/*", () =>
  Effect.gen(function* () {
    ...
    expect(Permission.evaluate("edit", ".open-coai/plans/foo.md", plan!.permission).action).toBe("allow")
  }),
)
...
it.instance(
  ...
  () =>
    Effect.gen(function* () {
      ...
      const skillDir = path.join(test.directory, ".open-coai", "skill", "perm-skill")
...
`,
      ...
      const home = process.env.OPEN_COAI_TEST_HOME
      process.env.OPEN_COAI_TEST_HOME = test.directory
