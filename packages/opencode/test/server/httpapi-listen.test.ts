...
const original = {
  ...
  envPassword: process.env.OPEN_COAI_SERVER_PASSWORD,
  envUsername: process.env.OPEN_COAI_SERVER_USERNAME,
}
...
afterEach(async () => {
  ...
  if (original.envPassword === undefined) delete process.env.OPEN_COAI_SERVER_PASSWORD
  else process.env.OPEN_COAI_SERVER_PASSWORD = original.envPassword
  if (original.envUsername === undefined) delete process.env.OPEN_COAI_SERVER_USERNAME
  else process.env.OPEN_COAI_SERVER_USERNAME = original.envUsername
  await disposeAllInstances()
  await resetDatabase()
})

async function startListener() {
  Flag.OPEN_COAI_SERVER_PASSWORD = auth.password
  Flag.OPEN_COAI_SERVER_USERNAME = auth.username
  process.env.OPEN_COAI_SERVER_PASSWORD = auth.password
  process.env.OPEN_COAI_SERVER_USERNAME = auth.username
  return Server.listen({ hostname: "127.0.0.1", port: 0 })
}

async function startNoAuthListener() {
  Flag.OPEN_COAI_SERVER_PASSWORD = undefined
  Flag.OPEN_COAI_SERVER_USERNAME = auth.username
  delete process.env.OPEN_COAI_SERVER_PASSWORD
  process.env.OPEN_COAI_SERVER_USERNAME = auth.username
  return Server.listen({ hostname: "127.0.0.1", port: 0 })
}
