  ...
  Effect.gen(function* () {
    ...
    yield* Effect.addFinalizer(() =>
      Effect.sync(() => {
        ...
        restoreEnv("OPEN_COAI_SERVER_PASSWORD", original.envPassword)
        restoreEnv("OPEN_COAI_SERVER_USERNAME", original.envUsername)
      }),
    )
  }),
)
      ...
      Layer.provide(
        ConfigProvider.layer(
          ConfigProvider.fromUnknown({
            OPEN_COAI_SERVER_PASSWORD: input?.password,
            OPEN_COAI_SERVER_USERNAME: input?.username,
          }),
        ),
      ),
    ),
