      ...
      Effect.flatMap((tmp) => {
        ...
        return Effect.gen(function* () {
          yield* Effect.promise(async () => {
            ...
            await fs.mkdir(path.join(root, ".open-coai"), { recursive: true })
            await fs.mkdir(path.join(directory, ".open-coai"), { recursive: true })
            await Promise.all([
              ...
              fs.writeFile(path.join(root, ".open-coai", "opencode.json"), JSON.stringify({ $schema: "root-dot" })),
              fs.writeFile(
                path.join(directory, ".open-coai", "opencode.jsonc"),
          ...
          return yield* Effect.gen(function* () {
            ...
            expect(entries.filter((entry) => entry.type === "directory").map((entry) => entry.path)).toEqual([
              ...
              AbsolutePath.make(path.join(root, ".open-coai")),
              AbsolutePath.make(path.join(directory, ".open-coai")),
            ])
